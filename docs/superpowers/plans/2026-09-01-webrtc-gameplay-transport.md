...
- Decision: represent stale negotiation ownership as a typed lifecycle cancellation and translate it only at the Socket.IO acknowledgement boundary.
  Rationale: supersession, detach, and disposal are expected effects of replacing session ownership, while peer negotiation failures remain genuine operational faults. The typed boundary avoids message matching, keeps stale activation false, returns a recoverable fallback signal to late callers, and preserves logging for real failures.
