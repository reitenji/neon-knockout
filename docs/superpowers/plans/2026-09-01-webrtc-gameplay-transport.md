...
- The corrected latency artifact measures browser ArenaInput sample time to the first browser-accepted authoritative snapshot whose local `lastProcessedInputSeq` reaches that exact sequence. It uses the same page/keyboard path, warms each mode, measures WebRTC first and the same page after forced fallback, and verifies each measured sequence's server-recorded source. The final full run measured WebRTC `10.20/11.30` ms median/p95 and Socket.IO `8.90/18.20` ms; four additional local runs measured `2.70/4.90` vs `18.00/24.20`, `13.00/29.60` vs `2.60/18.80`, `10.10/10.90` vs `18.30/27.60`, and `7.00/9.30` vs `18.30/25.70`. Results remain mixed and order-sensitive and do not establish physical-LAN improvement.
- The final full Playwright run passed 12/12. Frame median/p95 was `8.30/9.60` ms, and the four-hit/four-ring-out burst stayed at `10.30` ms maximum and `10.00` ms p95 with all 180 sampled frames accounted for. Three repeated dropped-frame browser runs and three repeated performance runs passed 6/6.
- Final gates passed independently and through `npm run verify`: lint; both TypeScript projects; 59 Vitest files with 581 tests; the 10-second eight-client Socket.IO fallback load test 1/1; and the production client/server build. `npm run test:e2e` separately passed 12/12 after its own production build.
