...
- Mobile Safari compatibility now has an isolated `mobile-webkit` Playwright project instead of user-agent spoofing or rerunning Chromium/CDP-specific coverage under another engine. Its smoke uses Playwright's WebKit touchscreen API, verifies trusted touch start/end on the real action control, and records the exact authoritative gameplay transport source. It proves WebKit engine behavior on this host, not a physical iPhone, iOS local-network permission, or representative LAN latency.
- The WebKit gate began RED because `mobile-webkit` did not exist, then exposed the absent browser runtime. Playwright WebKit 26.5 build 2336 was downloaded without dependency or lockfile changes. The final production-build script passed 1/1, a JSON-reported run passed 1/1 with `gameplay-transport: webrtc`, and the dedicated smoke passed 3/3 with `--repeat-each=3`; lint and both TypeScript projects remained green.
