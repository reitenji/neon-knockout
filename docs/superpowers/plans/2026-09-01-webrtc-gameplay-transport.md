...
- [x] (2026-09-02 13:08Z) Completed Task 8 review fix round at `bfc2d84` and `0865c09`: acknowledgement-gated input edges, all-client rematch generations, exact source/sequence evidence, fresh resume telemetry, real touch input, and abortable performance sampling; full automated acceptance is green while the physical LAN-device gate remains unperformed.
- [x] (2026-09-02 10:38Z) Completed Task 8 lifecycle fix round at `576c694`: superseded, detached, and stale negotiations now return recoverable transport unavailability without an unexpected server error, while genuine peer failures remain logged; two consecutive post-fix full Playwright runs passed 12/12.
- [x] (2026-09-02 10:54Z) Completed Task 8 semantic lifecycle follow-up at `83bbc62`: pre-room and stopped-session requests now use a distinct session-unavailable classification while retaining recoverable `TRANSPORT_UNAVAILABLE` with no unexpected log; full automated gates remain green.
- [x] (2026-09-02) Added the missing dedicated Playwright WebKit mobile smoke: a real WebKit engine with iPhone-like touch context creates, joins, and starts a production match, renders the landscape touch HUD, and correlates one trusted quick tap to its exact `quick: true` sequence, WebRTC server acceptance, and single authoritative `QUICK_1`. Physical iOS Safari remains a separate live-device gate.
...
- Observation: a healthy unordered `maxRetransmits: 0` channel can lose or reorder the only quick/dash-bearing frame even without a fallback transition, and a fixed two-successor repetition window is not sufficient.
  Evidence: a newer neutral sequence can overtake all repeated edges and permanently suppress them. Commit `bfc2d84` supersedes the fixed repetition scheme: it latches quick/dash across every successor until the accepted local-player snapshot acknowledges `lastProcessedInputSeq` at or beyond the edge. If acknowledgement does not arrive within 250 ms, or a fast send gate fails while an edge is pending, the generation switches wholly to Socket.IO and keeps latching there until an authoritative acknowledgement. A real Chromium test drops the first quick-bearing RTC frame and proves exactly one later WebRTC-owned action.
- Observation: reconnect publication order exposed stale transport telemetry before a resumed session attached its replacement transport.
  Evidence: the RED integration assertion observed `{ transport: 'webrtc', currentMs: 33, medianMs: 33 }` in the first peer-visible resume snapshot. `RoomManager.resume()` now receives the current Socket.IO transport and clears the player's samples before materializing that snapshot; the same assertion is GREEN with `websocket` and all three latency fields null.
...
- Observation: expected WebRTC negotiation supersession was indistinguishable from a genuine peer failure at the Socket.IO acknowledgement boundary.
  Evidence: the hub threw the same generic `Error` shape for a stale generation that `acknowledgeAsync` uses for unexpected faults, so an ordinary replacement race polluted the server error ledger and failed full E2E teardown. A deferred first negotiation plus a successful replacement reproduced `INTERNAL_ERROR` deterministically before commit `576c694`; the stale acknowledgement is now recoverable `TRANSPORT_UNAVAILABLE`, its generation cannot activate, and a forced peer failure still produces `INTERNAL_ERROR` plus one logger call.
- Observation: the first typed lifecycle fix classified a new negotiation with no attached room session as if an in-flight generation had been cancelled.
  Evidence: pre-room and post-stop hub calls were instances of `GameplayTransportNegotiationCancelledError`, although no negotiation existed to cancel. Commit `83bbc62` separates those calls as `GameplayTransportSessionUnavailableError`; both expected lifecycle subclasses retain the same no-log `TRANSPORT_UNAVAILABLE` socket contract.
...
- Decision: use a clean worktree based on the committed spec rather than modifying the abandoned prototype in place.
  ...
  Date/Author: 2026-09-01, Jarvis.
- Decision: latch transient quick/dash edges until an accepted authoritative snapshot acknowledges their input sequence; do not infer acknowledgement from attack state or a fixed send count.
  Rationale: monotonic snapshot acknowledgement survives arbitrary loss/reorder without turning a game-rule-rejected edge into a phantom action. When a pending edge cannot use the fast channel, the gameplay controller owns the Socket.IO successor itself so a neutral frame cannot cross transports and overtake it. Heavy release remains unlatched because later `heavy: false` samples already persist release semantics.
...
- Decision: represent expected transport lifecycle exits with a narrow base and distinct session-unavailable and in-flight-cancellation subclasses, then translate only the base at the Socket.IO acknowledgement boundary.
  Rationale: supersession, detach, and disposal cancel existing negotiation ownership; a pre-room or stopped request has no session and therefore no negotiation to cancel. Both are expected and recoverable, while peer negotiation failures remain genuine operational faults. The typed boundary avoids message matching, keeps stale activation false, returns a recoverable fallback signal, and preserves logging for real failures.
  Date/Author: 2026-09-02, Jarvis after full-suite lifecycle review.
...
- Real Chromium acceptance passed with both two-player contexts in `webrtc`, numeric Ping for both roster rows, authoritative movement/quick/heavy input, an in-place guest transition to `websocket`, continued authoritative quick input without navigation, lobby return, fresh generations for both clients, and WebRTC-owned rematch input from both. A context with `RTCPeerConnection` removed played through `websocket`; the existing WebSocket-to-polling gate remained intact. Mobile landscape touch advanced the authoritative sequence in an accepted `webrtc`, `websocket`, or `polling` mode.
...
- Review round RED/GREEN was explicit: acknowledgement latching started at 5 failed / 27 passed; authoritative rematch/ack integration at 3 failed / 18 passed; source recording at 1 failed / 7 passed; the browser observer at 1 failed / 5 skipped; and stale resume telemetry at 1 failed / 21 skipped. The complete focused set then passed 7 files / 113 tests, followed by lint and both TypeScript projects.
- The in-process harness now keeps at most 256 accepted-input records per player with exact sequence and `webrtc`/`websocket`/`polling` source, plus current WebRTC generation ID and negotiation count. Browser acceptance proves an exact quick sequence through WebRTC, the exact source change after forced fallback, changed generation IDs for both players on RESULT→LOBBY, WebRTC input from both players in the rematch, and no host double-negotiation.
- Mobile acceptance now dispatches Chromium `Input.dispatchTouchEvent` lifecycle events for joystick movement, quick, heavy, and dash, including touch end and cancel. Performance companion driving and frame samplers are explicitly stopped and awaited from `finally` paths.
- Mobile Safari compatibility now has an isolated `mobile-webkit` Playwright project instead of user-agent spoofing or rerunning Chromium/CDP-specific coverage under another engine. Its smoke uses Playwright's WebKit touchscreen API, verifies trusted touch start/end on the real action control, and matches the resulting quick input to the exact WebRTC-accepted sequence and one completed authoritative attack. It proves WebKit engine behavior on this host, not a physical iPhone, iOS local-network permission, or representative LAN latency.
- The WebKit gate began RED because `mobile-webkit` did not exist, then exposed the absent browser runtime. Playwright WebKit 26.5 build 2336 was downloaded without dependency or lockfile changes. Review tightening reproduced the production tap race as 2 failed / 7 passed focused tests and 1 failed WebKit smoke: trusted down/up both occurred between input samples, so no `quick: true` frame existed. The pending-edge implementation passed the 24-test focused input/control set, the production-build WebKit smoke 1/1, and the dedicated repeat 3/3; lint and both TypeScript projects remained green.
- The corrected latency artifact measures browser ArenaInput sample time to the first browser-accepted authoritative snapshot whose local `lastProcessedInputSeq` reaches that exact sequence. It uses the same page/keyboard path, warms each mode, measures WebRTC first and the same page after forced fallback, and verifies each measured sequence's server-recorded source. The two post-lifecycle-fix full runs measured WebRTC `11.90/21.40` and `4.70/17.40` ms median/p95 versus Socket.IO `12.70/26.00` and `9.80/26.40` ms. An isolated performance rerun measured `18.40/20.20` vs `5.90/19.50` ms. These add to the earlier mixed local samples; the order-sensitive same-host evidence still does not establish physical-LAN improvement.
- The reviewer reproduced the pre-fix logger leak in two 11/12 full Playwright runs. After commit `576c694`, the first full run reached 11/12 for a different reason: the performance browser remained on `websocket` instead of activating WebRTC. That exact performance test passed immediately in isolation, then two consecutive full runs passed 12/12 without an unexpected logger entry. The retained post-fix frame median/p95 values were `8.30/10.10` and `8.30/10.00` ms; four-hit/four-ring-out maximum/p95 values were `16.60/9.70` and `26.70/9.90` ms with all 180 frames accounted for. No threshold or retry policy changed.
- Lifecycle RED/GREEN was explicit: a deferred first generation superseded by a successful second generation returned `INTERNAL_ERROR` and failed 1/1 with 22 tests skipped. The typed-cancellation implementation passed 3/3 selected lifecycle/error-path tests, then both complete hub/socket integration files passed 44/44. Stale generations remain unable to activate; a genuine forced negotiation error remains `INTERNAL_ERROR` and is explicitly asserted to log once.
- Fix-round-2 gates passed independently and through `npm run verify`: lint; both TypeScript projects; 59 Vitest files with 583 tests; the 10-second eight-client Socket.IO fallback load test 1/1; and the production client/server build. `npm run test:e2e` passed 12/12 in two consecutive full runs after its own production build.
- The semantic lifecycle follow-up RED observed the cancellation class for unattached/stopped requests (1 failed / 3 passed / 42 skipped). GREEN was 4/4 selected and 46/46 across the complete hub/socket integration files. The latest `npm run verify` passed lint, both TypeScript projects, 59 files / 585 tests, load 1/1, and production build; the subsequent full Playwright run passed 12/12 with no unexpected logger errors.
